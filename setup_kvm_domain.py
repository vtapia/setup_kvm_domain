...
def read_args():
    ...
    parser.add_argument('-r', '--restart', help='Restart VM to apply changes.', action="store_true")
    parser.add_argument('-v', '--verbose', help='Show debug messages', action="store_true")
    parser.add_argument('-d', '--dump', help='Dump the new domain XML but do not apply it.', action="store_true")
    parser.add_argument('-m', '--memory', type=int,  help='Memory in MB.')
    parser.add_argument('-c', '--cpu', type=int,  help='vCPU number.')
    parser.add_argument('-q', '--queues', type=int,  help='virtqueues number.')
    parser.add_argument('-p', '--pin', type=pinning_arg, help='CPU pinning in [0-9]-[0-9] format (e.g. "1-8").\
    ...
    parser.add_argument('vm', type=str, help='VM name')
    args = parser.parse_args()

    logger.debug("- Using config file: %s" % args)
    return args
